...
resource "aws_network_acl" "bus_log_network_acl_availability_zone_2" {
  vpc_id = aws_vpc.prod_vpc.id
  subnet_ids = [aws_subnet.private_subnet_1 , aws_subnet.private_subnet_3.id]
  ...
  ingress {
    ...
    rule_no    = 101
    action     = "allow"
    cidr_block = ["10.0.1.0/24","10.0.4.0/24"]
    from_port  = 80
    to_port    = 80
  }
...
resource "aws_network_acl" "database_network_acl_availability_zone_2" {
  vpc_id = aws_vpc.prod_vpc.id
  subnet_ids = [aws_subnet.privte_subnet_2.id , aws_subnet.private_subnet_4.id]
  ...
  ingress {
    ...
    rule_no    = 102
    action     = "allow"
    cidr_block = ["10.0.2.0/24","10.0.5.0/24"]
    from_port  = 3306
    to_port    = 3306
  }
